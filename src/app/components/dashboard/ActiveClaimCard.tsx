...
const TRIGGER_LABELS: Record<string, string> = {
  ...
  curfew: 'Zone Lockdown', platform: 'Platform Outage', pandemic: 'Pandemic Lockdown',
}

export const ActiveClaimCard = memo(function ActiveClaimCard({ claim }: { claim: ActiveClaim }) {
  ...
  return (
    <div className={`border-2 rounded-xl p-6 ${statusColor}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div><p className="text-xs text-gray-600 mb-1">Trigger</p><p className="font-semibold text-gray-900">{triggerLabel}</p></div>
        <div><p className="text-xs text-gray-600 mb-1">Income Loss</p><p className="font-semibold text-gray-900">₹{(income_loss ?? 0).toLocaleString('en-IN')}</p></div>
        <div><p className="text-xs text-gray-600 mb-1">Income Loss Payout</p><p className="font-semibold text-gray-900">₹{(payout_amount ?? 0).toLocaleString('en-IN')}</p></div>
        <div>
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        ...
        {status === 'PAID' && <span className="text-xs font-medium text-[#06C167]">✓ Income loss payout completed</span>}
      </div>
    </div>
  )
})
