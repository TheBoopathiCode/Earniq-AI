  ...
  return (
    <header className="bg-white border-b border-gray-200 px-4 lg:px-6 py-3 lg:py-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 lg:gap-4 flex-wrap pl-10 lg:pl-0">
          <div>
            <p className="text-xs text-gray-500">{t('worker')}</p>
            <p className="font-semibold text-gray-900 text-sm">{worker?.name || worker?.phone || '—'}</p>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="relative p-2 text-gray-600 hover:bg-gray-100 rounded-lg">
            ...
            {dcsScore >= 40 && <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full" />}
          </button>
        </div>
      </div>
    </header>
